/* A portunun Pin_0 ve Pin_1 bacaklarina bagli potansiyometrelerden 
   ADC1 cevresel birimiyle veriyi alip cpu yu kullanmadan dogrudan bellege yaziyoruz */

#include "stm32f4xx.h"
#include "stm32f4_discovery.h"

#define BufferLength 2 // iki adet ADC okumasi yapacagimiz icin 2 degerini verdik. 
...
void ADC_Config() {
	...
	ADC_InitStruct.ADC_ScanConvMode = ENABLE;	    // Birden fazla ADC okumasi yapildigi icin tarama modu aktif edilmeli.
	...
	ADC_InitStruct.ADC_ExternalTrigConv = 0;	    // cevrim tetigi yok dedik, harici tetikleme olmasın
	ADC_InitStruct.ADC_DataAlign = ADC_DataAlign_Right; // data yazma islemi saga yaslı
	...
	ADC_RegularChannelConfig(ADC1,ADC_Channel_0, 1, ADC_SampleTime_3Cycles); // rank degeri 1 yani 1. sirada buna gore once 0 pindeki ADC islemi yapilacak
	ADC_RegularChannelConfig(ADC1,ADC_Channel_1, 2, ADC_SampleTime_3Cycles); // rank degeri 2 yani 2. sirada 0 pinden sonra 1 pindeki ADCislemi yapilacak

	ADC_DMARequestAfterLastTransferCmd(ADC1,ENABLE); // Her ADC DMA transferinden sonra yeni transferler icin istek yapiyoruz.MUTLAKA OLMALI
...
void DMA_Config() {
	...
	DMA_InitStruct.DMA_MemoryInc = ENABLE; 				// hafızaya yazma işleminde adresin surekli degismesini saglıyoruz .cunku surekli ayni yere yazarsa eski degerler kaybolacak
	DMA_InitStruct.DMA_PeripheralInc = DISABLE;			// ancak veri okudugumuz adresin surekli sabit kalmasini sagliyoruz.cunku okudugumuz adres hic degismiyor
...
int main(void) {
	...
	/* ADC_SoftwareStartConv islemini while icinde yapmadık cunku while icinde yaparsak cevresel birim -> CPU -> RAM seklinde
	 * yazmıs oluruz. ama burada direk olarak cevresel birim -> RAM  seklinde bir yazma gerceklestirdik.  */
  while (1) {
	  adc_value1 = adc_value[0]; // stm studio da degisken degerlerini izleyebilmek icin her 2 ADC kanali icin olusturduk
